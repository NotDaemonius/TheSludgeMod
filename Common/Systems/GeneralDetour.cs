...
namespace TheSludgeMod.Common.Systems
{
    public class GeneralDetour : ModSystem
    {
        ...
        {
            _roseQuartzAsset = ModContent.Request<Texture2D>(
                "TheSludgeMod/Content/Items/RoseQuartz/GiantRoseQuartz",
        ...
        {
            _giantRoseQuartzType = ModContent.ItemType<GiantRoseQuartz>();
        }

        private static void On_PlayerDrawLayers_DrawPlayer_36_CTG(
            On_PlayerDrawLayers.orig_DrawPlayer_36_CTG orig,
            ref PlayerDrawSet drawinfo)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                Texture2D texture = (j == 7)
                    ? _roseQuartzAsset.Value
                    : TextureAssets.Gem[j].Value;

                drawinfo.DrawDataCache.Add(new DrawData(
                    texture,
                    centerPos + direction * orbitRadius,
                    sourceRect: null,
                    color: gemColor,
                    rotation: 0f,
