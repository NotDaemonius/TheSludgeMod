...
public class ModifyAlterOreSpawns : ModSystem
{
    ...
    private void OnSmashAltar(On_WorldGen.orig_SmashAltar orig, int x, int y)
    {
        ...
        if (flag) NetMessage.SendData(MessageID.WorldData);
        Color color = new Color(50, 255, 130);
        string message = $"Your world has been blessed with {vanillaName}, {altName}, and {thirdName}!";
        if (Main.netMode == NetmodeID.SinglePlayer) Main.NewText(message, color.R, color.G, color.B);
        else if (Main.netMode == NetmodeID.Server) ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral(message), color);
        ...
        void RunOre(int type, double oreCount)
        {
            ...
            while ((double)n < oreCount)
            {
                ...
                if (type == TileID.Mythril || type == TileID.Orichalcum || type == osmium) minDepth = Main.rockLayer;
                if (type == TileID.Adamantite || type == TileID.Titanium || type == bismuth) minDepth = (Main.rockLayer + Main.rockLayer + (double)Main.maxTilesY) / 3.0;
                int j2 = WorldGen.genRand.Next((int)minDepth, Main.maxTilesY - 150);

                if (Main.remixWorld)
                {
                    double remixMin = Main.maxTilesX - 350;
                    if (type == TileID.Mythril || type == TileID.Orichalcum || type == osmium) remixMin = (Main.rockLayer + Main.rockLayer + (double)Main.maxTilesY - 350.0) / 3.0;
                    if (type == TileID.Adamantite || type == TileID.Titanium || type == bismuth) remixMin = Main.rockLayer - 25.0;
                    j2 = WorldGen.genRand.Next((int)Main.worldSurface + 15, (int)remixMin);
                }

                WorldGen.OreRunner(i2, j2, WorldGen.genRand.Next(5, strengthMax), WorldGen.genRand.Next(5, strengthMax), (ushort)type);
                n++;
            }
        }
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            for (int k = 0; k < wraiths; k++) NPC.SpawnOnPlayer((int)Player.FindClosest(new Vector2(x * 16, y * 16), 16, 16), NPCID.Wraith);
        }
