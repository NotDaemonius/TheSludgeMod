...
public class OreGenSystem : ModSystem
{
    public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
    {
        ...
        if (idx != -1) tasks.Insert(idx + 1, new PassLegacy("Generate Alternate Ores", GenerateAlternateOres));
    }

    private void GenerateAlternateOres(GenerationProgress progress, GameConfiguration config)
    {
        if (WorldGen.drunkWorldGen) return;
        ...
        {
            for (int i = 0; i < count; i++) WorldGen.TileRunner(WorldGen.genRand.Next(x1, x2), WorldGen.genRand.Next(y1, y2), WorldGen.genRand.Next(sMin, sMax), WorldGen.genRand.Next(stMin, stMax), type, false, 0f, 0f, false, true);
        }
