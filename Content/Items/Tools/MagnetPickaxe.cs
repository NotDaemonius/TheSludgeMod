    ...
    public override void HoldItem(Player player)
    {
        foreach (Item item in Main.ActiveItems)
        {
            if (item.noGrabDelay == 0 && item.playerIndexTheItemIsReservedFor == player.whoAmI && (player.Center - item.Center).Length() < 400)
            {
                ...
                {
                    if (item.velocity.X < 90f + player.velocity.X) item.velocity.X += 9f;
                    if (item.velocity.X < 0f) item.velocity.X += 9f * 0.75f;
                }
                else
                {
                    if (item.velocity.X > -90f + player.velocity.X) item.velocity.X -= 9f;
                ...
                {
                    if (item.velocity.Y < 90f) item.velocity.Y += 9f;
                    if (item.velocity.Y < 0f) item.velocity.Y += 9f * 0.75f;
                }
                else
                {
                    if (item.velocity.Y > -90f) item.velocity.Y -= 9f;
