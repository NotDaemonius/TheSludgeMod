...
public class DartInABottle : ModItem
{
    ...
    public override void UpdateAccessory(Player player, bool hideVisual) => player.GetJumpState<SimpleExtraJump>().Enable();
...
public class SimpleExtraJump : ExtraJump
{
    ...
    public override void OnStarted(Player player, ref bool playSound)
    {
        ...
        if (player.gravDir == -1f) offsetY = 0;
        offsetY -= 16;
    ...
    private static void SpawnCloudPoof(Player player, Vector2 position) => Projectile.NewProjectile(player.GetSource_FromThis(), player.Bottom, new Vector2(0, 10), ProjectileID.PoisonDart, 21, 3, -1);

    public override void ShowVisuals(Player player)
    {
        int offsetY = player.height - 6;
        if (player.gravDir == -1f) offsetY = 6;
