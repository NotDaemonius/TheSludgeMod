...
public class RichMahoganyCrystalProj : ModProjectile
{
    private const float HoverHeight = 36f;
    private const float DetectRange = 600f;
    private const int ShootInterval = 152;
    ...
    private bool CheckActive(Player owner)
    {
        ...
        {
            owner.ClearBuff(ModContent.BuffType<RichMahoganyCrystalBuff>());
            Projectile.active = false;
            return false;
        }

        if (owner.HasBuff(ModContent.BuffType<RichMahoganyCrystalBuff>())) Projectile.timeLeft = 2;
        return true;
    }
    ...
    private void FireLeaf(NPC target)
    {
        ...
        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, direction * LeafSpeed, ModContent.ProjectileType<RichMahoganyTreeWandProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
    }
}