...
public class ShadewoodCrystalProj : ModProjectile
{
    private const float HoverHeight = 36f;
    private const float DetectRange = 600f;
    private const int ShootInterval = 144;
    ...
    private bool CheckActive(Player owner)
    {
        ...
        {
            owner.ClearBuff(ModContent.BuffType<ShadewoodCrystalBuff>());
            Projectile.active = false;
            return false;
        }

        if (owner.HasBuff(ModContent.BuffType<ShadewoodCrystalBuff>())) Projectile.timeLeft = 2;
        return true;
    }
    ...
    private void FireLeaf(NPC target)
    {
        ...
        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, direction * LeafSpeed, ModContent.ProjectileType<ShadewoodTreeWandProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
    }
}