...
namespace TheSludgeMod.Content.Projectiles.Weapons
{
    public class CthulhuEyeStaffProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (owner.HasBuff(ModContent.BuffType<Buffs.CthulhuEyeBuff>()))
            ...
            if (tooFar)
                _returning = true;
            else if (distToOwner <= MAX_WANDER_DIST)
                _returning = false;
        ...
        private NPC FindTarget(Player owner)
        {
            if (owner.HasMinionAttackTargetNPC)
            {
                ...
                if (manual.CanBeChasedBy() && Vector2.Distance(Projectile.Center, manual.Center) <= DETECT_RANGE)
                    return manual;
            }
