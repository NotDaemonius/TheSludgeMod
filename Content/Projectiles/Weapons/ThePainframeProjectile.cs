...
namespace TheSludgeMod.Content.Projectiles.Weapons
{
	...
	{
		private const string ChainTexturePath = "TheSludgeMod/Content/Projectiles/Weapons/ThePainframeChain"; // The folder path to the flail chain sprite
		private const string ChainTextureExtraPath = "TheSludgeMod/Content/Projectiles/Weapons/ThePainframeChainExtra";  // This texture and related code is optional and used for a unique effect
		...
		public override void AI() {
			...
			int spinHitCooldown = 16; // How often your flail hits when spinning
			int movingHitCooldown = 8; // How often your flail hits when moving
			...
			switch (CurrentAIState) {
				case AIState.Spinning: {
						...
						if (Projectile.owner == Main.myPlayer) {
                            ...
                            // Fire a laser toward the cursor every 12 ticks while spinning
                            if (Projectile.owner == Main.myPlayer && SpinningStateTimer > 12f)
                            {
                                ...
                                if (Math.Sign(prevDiff) != Math.Sign(currDiff))
                                {
                                    ...
                                    int projIndex = Projectile.NewProjectile(
                                        ...
                                        Projectile.Center,
                                        laserVelocity,
                                        ModContent.ProjectileType<TheMainframeLaser>(),
                                        40,
			...
			*/
			...
			if (Main.rand.NextBool(dustRate))
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Silver, 0f, 0f, 150, default(Color), 1.3f);
		}
