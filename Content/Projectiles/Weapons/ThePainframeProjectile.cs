...
namespace TheSludgeMod.Content.Projectiles.Weapons
{
	...
	public class ThePainframeProjectile : ModProjectile
	{
		private const string ChainTexturePath = "TheSludgeMod/Content/Projectiles/Weapons/ThePainframeChain";
		private const string ChainTextureExtraPath = "TheSludgeMod/Content/Projectiles/Weapons/ThePainframeChainExtra";
		private static Asset<Texture2D> chainTexture;
		private static Asset<Texture2D> chainTextureExtra;

        private enum AIState { Spinning, LaunchingForward, Retracting, UnusedState, ForcedRetracting, Ricochet, Dropping }
        ...
        public override void AI()
        {
            ...
            switch (CurrentAIState)
            {
                case AIState.Spinning:
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer && SpinningStateTimer > 12f)
                        {
                            float mouseAngle = mountedCenter.DirectionTo(Main.MouseWorld).ToRotation();
                            float prevSpinAngle = (float)(Math.PI * 10f * ((SpinningStateTimer - 1f) / 60f) * player.direction);
                            float currSpinAngle = (float)(Math.PI * 10f * (SpinningStateTimer / 60f) * player.direction);
                            float prevDiff = MathHelper.WrapAngle(prevSpinAngle - mouseAngle);
                            float currDiff = MathHelper.WrapAngle(currSpinAngle - mouseAngle);

                            if (Math.Sign(prevDiff) != Math.Sign(currDiff))
                            {
                                Vector2 directionToMouse = mountedCenter.DirectionTo(Main.MouseWorld).SafeNormalize(Vector2.UnitX * player.direction);
                                Vector2 laserVelocity = directionToMouse * 12f;
                                int projIndex = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, laserVelocity, ModContent.ProjectileType<TheMainframeLaser>(), 40, Projectile.knockBack, Main.myPlayer);
                ...
                case AIState.LaunchingForward:
                {
                    ...
                    if (player.controlUseItem)
                    {
						...
						for (int i = 0; i < numberProjectiles; i++) {
							Vector2 perturbedSpeed = Vector2.UnitX.RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numberProjectiles - 1))) * 8f;
                            int projIndex = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, perturbedSpeed, ModContent.ProjectileType<TheMainframeLaser>(), 40, Projectile.knockBack, Main.myPlayer);
                            Main.projectile[projIndex].friendly = true;
                            Main.projectile[projIndex].hostile = false;
                            Main.projectile[projIndex].DamageType = DamageClass.Ranged;
                            Main.projectile[projIndex].usesLocalNPCImmunity = true;
            ...
            int dustRate = 15;
            if (doFastThrowDust) dustRate = 1;
            if (Main.rand.NextBool(dustRate)) Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Silver, 0f, 0f, 150, default(Color), 1.3f);
        }
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            {
                Projectile.netUpdate = true;
                for (int i = 0; i < impactIntensity; i++) Collision.HitTiles(Projectile.position, velocity, Projectile.width, Projectile.height);
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            if (CurrentAIState == AIState.Spinning) modifiers.Knockback *= 0.25f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            while (chainLengthRemainingToDraw > 0f)
            {
                ...
                if (chainCount >= 4) ;
                else if (chainCount >= 2) chainTextureToDraw = chainTextureExtra;
                else chainTextureToDraw = chainTextureExtra;
