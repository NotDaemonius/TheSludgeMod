...
public class AshWoodCrystalProj : ModProjectile
{
    private const float HoverHeight = 36f;
    private const float DetectRange = 600f;
    private const int ShootInterval = 136;
    private const float LeafSpeed = 12f;
    ...
    private bool CheckActive(Player owner)
    {
        ...
        {
            owner.ClearBuff(ModContent.BuffType<AshWoodCrystalBuff>());
            Projectile.active = false;
            return false;
        }

        if (owner.HasBuff(ModContent.BuffType<AshWoodCrystalBuff>())) Projectile.timeLeft = 2;
        return true;
    }
    ...
    private void FireLeaf(NPC target)
    {
        ...
        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, direction * LeafSpeed, ModContent.ProjectileType<AshWoodTreeWandProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
    }
}