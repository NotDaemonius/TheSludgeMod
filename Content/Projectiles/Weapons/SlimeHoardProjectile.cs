...
namespace TheSludgeMod.Content.Projectiles.Weapons
{
    public class SlimeHoardProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            bool rightyy = false;
            bool walled = false;
            bool typevar = Projectile.ai[0] == -1f || Projectile.ai[0] == -2f;
            bool pullRight = Projectile.ai[0] == -1f;
            bool pullStop = Projectile.ai[0] == -2f;
            int buffType = ModContent.BuffType<Buffs.SlimeHoardBuff>();
            if (owner.dead) owner.ClearBuff(buffType);
            if (owner.HasBuff(buffType) || typevar) Projectile.timeLeft = 2;
            ...
            if (Projectile.ai[1] == 0f && !typevar)
            {
                ...
                if (dist > 2000f) Projectile.Center = owner.Center;
            ...
            if (Projectile.ai[0] != 0f && !typevar)
            {
                ...
                if (dist < 100 && owner.velocity.Y == 0f && Projectile.position.Y + Projectile.height <= owner.position.Y + owner.height && !Collision.SolidCollision(Projectile.position, Projectile.width, Projectile.height))
                {
                    Projectile.ai[0] = 0f;
            ...
            else
            {
                ...
                if (pointed != null && pointed.CanBeChasedBy(Projectile, false))
                {
                    ...
                    if (d < bestDist)
                    {
                        ...
                        if (Collision.CanHit(Projectile.position, Projectile.width, Projectile.height, pointed.position, pointed.width, pointed.height)) { bestDist = d; targetX = pointed.Center.X; targetY = pointed.Center.Y; targetIdx = pointed.whoAmI; }
                    }
                }
                ...
                if (targetIdx == -1)
                {
                    for (int i = 0; i < Main.maxNPCs; i++)
                    {
                        ...
                        if (d < bestDist)
                        {
                            ...
                            if (Collision.CanHit(Projectile.position, Projectile.width, Projectile.height, Main.npc[i].position, Main.npc[i].width, Main.npc[i].height)) { bestDist = d; targetX = Main.npc[i].Center.X; targetY = Main.npc[i].Center.Y; targetIdx = i; }
                        }
                    }
                }
            ...
            else Projectile.velocity.Y += 0.4f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (color == Color.AliceBlue) color = Main.hslToRgb(Main.rand.NextFloat(), 1f, Main.rand.NextFloat()) * Projectile.Opacity;
