...
public class BouncySquareBombProjectile : ModProjectile
{
    ...
    {
        if (Projectile.velocity.X != oldVelocity.X) Projectile.velocity.X = -oldVelocity.X * 0.95f;
        if (Projectile.velocity.Y != oldVelocity.Y) Projectile.velocity.Y = -oldVelocity.Y * 0.95f;
        return false;
    }        
    public override void OnKill(int timeLeft)
    {
        ...
        for (int x = centerTileX - ExplosionRadius; x <= centerTileX + ExplosionRadius; x++)
        {
            ...
            {
                if (!WorldGen.InWorld(x, y, fluff: 1)) continue;
                Tile tile = Main.tile[x, y];
                if (tile == null) continue;
                if (tile.HasTile && Main.tileDungeon[tile.TileType]) continue;
                if (tile.HasTile && (tile.TileType == TileID.LihzahrdBrick || tile.TileType == TileID.DemonAltar)) continue;
