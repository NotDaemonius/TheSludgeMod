...
public class Chandeliers : ModTile
{
    ...
    public override void Unload() => flameTexture = null;
    ...
    public override IEnumerable<Item> GetItemDrops(int i, int j)
    {
        ...
        if (itemType != 0) yield return new Item(itemType);
    }
    ...
    public override void HitWire(int i, int j)
    {
        ...
        if (Main.netMode != NetmodeID.SinglePlayer) NetMessage.SendTileSquare(-1, topX, topY, 3, 3);
    }

    public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
    {
        if (Main.tile[i, j].TileFrameX >= 54) return;
        r = 1f;
        g = 0.95f;
        b = 0.8f;
    }
    ...
    {
        Tile tile = Main.tile[i, j];
        if (TileObjectData.IsTopLeft(tile)) Main.instance.TilesRenderer.AddSpecialPoint(i, j, TileDrawing.TileCounterType.MultiTileVine);
        return false;
    }
